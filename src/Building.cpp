...
int buildingsDist(const Building & b1, const Building & b2) {
    ...
    int dist = INT_MAX;
    for (size_t row1 = b1Y; row1 < b1Plan.size(); row1++){
        for (size_t col1 = b1X; col1 < b1Plan[0].size(); col1++){
            if(b1Plan[row1][col1] == '.')
                continue;
            for (size_t row2 = b2Y; row2 < b2Plan.size(); row2++){
                for (size_t col2 = b2X; col2 < b2Plan[0].size(); col2++){
                    if(b2Plan[row2][col2] == '.')
                        continue;
                    dist = min(dist, manhattanDistance(row1,col1,row2,col2));
                }
